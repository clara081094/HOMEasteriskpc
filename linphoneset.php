...
//$ip="192.168.1.50";
echo shell_exec("linphonecsh init");
echo shell_exec("sleep 2");
echo shell_exec("linphonecsh generic 'ports sip 5061'");
echo shell_exec("linphonecsh generic 'autoanswer enable'");
echo shell_exec("linphonecsh register --host ".$ip." --username 11 --password password11");
echo shell_exec("linphonecsh soundcard playback 3");
echo shell_exec("linphonecsh soundcard capture 3");
$errno = "admin";
$errstr = "amp111";
$timeout = "30";
$rptan=0;
 ...
 while (!feof($socket)) {
    ...
    for($i=0;$i<sizeof($lines);$i++)
    {
         ...
         if(($rptan==0) AND ($rptac==1))
	 {
                ...
                echo shell_exec("linphonecsh dial 69@".$ip);
		echo "corrio";
	 }
        }
        $vari="";
